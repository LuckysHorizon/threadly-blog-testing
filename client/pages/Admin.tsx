...
import { 
  BarChart3, Users, FileText, MessageCircle, TrendingUp, Eye, Heart,
  CheckCircle, XCircle, Edit, Trash2, Clock, Crown, Search, Filter,
  MoreHorizontal, Calendar, ArrowUp, ArrowDown, AlertTriangle
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { useAuth } from "@/contexts/AuthContext";
import { mockBlogPosts, mockAuthors, getAuthorById, Author, BlogPost } from "@/lib/mockData";
...
export default function Admin() {
  ...
  const adminStats: AdminStats = {
    ...
    publishedBlogs: mockBlogPosts.filter(p => p.status === "published").length,
    ...
    totalComments: mockBlogPosts.reduce((sum, post) => sum + post.stats.comments, 0),
  ...
  // Handle blog actions
  const handleBlogAction = async (blogId: string, action: "approve" | "reject" | "delete") => {
    // Simulate API call
  ...
  // Handle user actions
  const handleUserAction = async (userId: string, action: "promote" | "demote" | "block" | "unblock") => {
    // Simulate API call
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-400 text-sm">Manage your AI Blog platform</p>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[
                ...
                { label: "Published Blogs", value: adminStats.publishedBlogs, icon: FileText, color: "green" },
                { label: "Pending Approval", value: adminStats.pendingBlogs, icon: Clock, color: "yellow" },
                { label: "Total Views", value: adminStats.totalViews.toLocaleString(), icon: Eye, color: "purple" }
              ].map((stat, index) => (
                <div key={index} className="glass-card p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className={`p-3 rounded-xl bg-gradient-to-r ${
                      ...
                      stat.color === "yellow" ? "from-yellow-500/20 to-orange-500/20" :
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <div className="glass-card p-6">
                ...
                <div className="space-y-3">
                  {mockBlogPosts.filter(p => p.status === "pending").map((post) => {
                    const author = getAuthorById(post.authorId);
                    return (
                      <div key={post.id} className="flex items-center justify-between p-3 rounded-lg bg-white/5">
                        <div>
                          <p className="text-white text-sm font-medium truncate">{post.title}</p>
                          <p className="text-gray-400 text-xs">by {author?.name}</p>
                        </div>
                        <div className="flex space-x-2">
                          <Button
                            size="sm"
                            onClick={() => handleBlogAction(post.id, "approve")}
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleBlogAction(post.id, "reject")}
              ...
              <div className="glass-card p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-blue-400 font-medium">{adminStats.newUsersThisMonth}</span>
                  </div>
                  <div className="flex items-center justify-between">
          ...
          <div className="space-y-6">
            ...
            <div className="glass-card p-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                ...
                <p className="text-gray-400 text-sm">{filteredBlogs.length} blog posts</p>
              </div>
            </div>
                    ...
                    {filteredBlogs.map((blog) => {
                      ...
                      return (
                        <tr key={blog.id} className="border-b border-white/5 hover:bg-white/5">
                          ...
                          <td className="p-4 text-white text-sm">{blog.stats.views.toLocaleString()}</td>
                          <td className="p-4 text-gray-400 text-sm">{blog.publishedAt}</td>
                          <td className="p-4">
                            <div className="flex items-center space-x-2">
                              {blog.status === "pending" && (
                                <>
        ...
        {activeTab === "comments" && (
          <div className="space-y-6">
            {/* Comments List */}
            <div className="glass-card p-6">
              <h3 className="text-lg font-semibold text-white mb-4">Comment Moderation</h3>
              <div className="space-y-4">
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-gray-400 text-xs">
                            On: <span className="text-brand-400">{comment.blogTitle}</span>
                          </p>
                        </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-white/10">
                      ...
                      <th className="text-left p-4 text-gray-400 font-medium">Articles</th>
                  ...
                  <tbody>
                    {filteredUsers.map((author) => (
                      <tr key={author.id} className="border-b border-white/5 hover:bg-white/5">
                        <td className="p-4">
                          <div className="flex items-center space-x-3">
                            <img
                              src={author.avatar}
                        ...
                        <td className="p-4 text-white text-sm">{author.articlesCount}</td>
                        <td className="p-4 text-white text-sm">{author.followersCount.toLocaleString()}</td>
                        <td className="p-4 text-gray-400 text-sm">{author.joinedAt}</td>
                        <td className="p-4">
                          <div className="flex items-center space-x-2">
                            <Button
                              size="sm"
                              onClick={() => handleUserAction(author.id, author.role === "admin" ? "demote" : "promote")}
                              className="glass-button text-gray-400 hover:text-white px-2 py-1"
                            >
              ...
              <div className="glass-card p-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-white font-medium">{adminStats.totalViews.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-400">Total Likes</span>
                    <span className="text-white font-medium">{adminStats.totalLikes.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-400">Total Comments</span>
                    <span className="text-white font-medium">{adminStats.totalComments.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    .map((post, index) => (
                      <div key={post.id} className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <span className="text-white text-sm truncate max-w-xs">{post.title}</span>
                        </div>
                        <span className="text-brand-400 text-sm">{post.stats.views.toLocaleString()} views</span>
                      </div>
                    ))}
                </div>
              </div>
